...
-- ═════════════════════════════════════════════════════════════
-- Round 3a (2026-07-10): дедуп-бэкстоп на уровне БД.
-- Применить отдельно через Supabase SQL Editor.
--
-- НЕ plain unique(name, year, kind): год почти всегда NULL (форма
-- добавления больше не спрашивает год), а Postgres считает NULL ≠
-- NULL в уникальных индексах — такое ограничение тихо ничего бы не
-- ловило. coalesce(year, 0) нормализует "год неизвестен" в одно и
-- то же значение, чтобы дубли без года тоже ловились. lower(name)
-- — регистронезависимость ("Матрица" = "матрица").
--
-- Если применение упадёт на unique_violation — значит в данных уже
-- есть дубли; найди и удали их вручную перед повторной попыткой:
--   select lower(name), kind, coalesce(year,0), count(*)
--   from public.titles group by 1,2,3 having count(*) > 1;
-- ═════════════════════════════════════════════════════════════
create unique index titles_dedup_idx
  on public.titles (lower(name), kind, coalesce(year, 0));
